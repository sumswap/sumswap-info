...
export function getSwapLink(token0Address, token1Address = null) {
  if (!token1Address) {
    return `https://swap.sumswap.org/#/swap?inputCurrency=${token0Address}`
  } else {
    return `https://swap.sumswap.org/#/swap?inputCurrency=${token0Address === '0xc02aaa39b223fe8d0a0e5c4f27ead9083c756cc2' ? 'ETH' : token0Address
      }&outputCurrency=${token1Address === '0xc02aaa39b223fe8d0a0e5c4f27ead9083c756cc2' ? 'ETH' : token1Address}`
  }
}

export function getMiningPoolLink(token0Address) {
  return `https://swap.sumswap.org/#/uni/ETH/${token0Address}`
}

export function getSumswapAppLink(linkVariable) {
  let baseSumswapUrl = 'https://swap.sumswap.org/#/'
  if (!linkVariable) {
    return baseSumswapUrl
  }

  return `${baseSumswapUrl}/ETH/${linkVariable}`
}
