...
function init() {
  ...
  try {
    const parsed = JSON.parse(window.localStorage.getItem(SUMSWAP))
...
export function Updater() {
  ...
  useEffect(() => {
    window.localStorage.setItem(SUMSWAP, JSON.stringify({ ...state, [LAST_SAVED]: Math.floor(Date.now() / 1000) }))
  })
