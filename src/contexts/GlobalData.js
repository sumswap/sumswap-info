...
const UPDATE_ALL_PAIRS_IN_SUMSWAP = 'UPDAUPDATE_ALL_PAIRS_IN_SUMSWAPTE_TOP_PAIRS'
const UPDATE_ALL_TOKENS_IN_SUMSWAP = 'UPDATE_ALL_TOKENS_IN_SUMSWAP'
...
function reducer(state, { type, payload }) {
  switch (type) {
    ...
    case UPDATE_ALL_PAIRS_IN_SUMSWAP: {
    ...
    case UPDATE_ALL_TOKENS_IN_SUMSWAP: {
...
export default function Provider({ children }) {
  ...
  const updateAllPairsInSumswap = useCallback((allPairs) => {
    dispatch({
      type: UPDATE_ALL_PAIRS_IN_SUMSWAP,
      payload: {
        allPairs,
      },
    })
  ...
  const updateAllTokensInSumswap = useCallback((allTokens) => {
    dispatch({
      type: UPDATE_ALL_TOKENS_IN_SUMSWAP,
      payload: {
        allTokens,
      },
    })
      ...
      value={useMemo(
        () => [
          state,
          {
            ...
            updateEthPrice,
            updateTopLps,
            updateAllPairsInSumswap,
            updateAllTokensInSumswap,
          },
        ],
        [
          state,
...
async function getGlobalData(ethPrice, oldEthPrice) {
  ...
  try {
    ...
    data = result.data.sumswapFactories[0]
    ...
    oneDayData = oneDayResult.data.sumswapFactories[0]

    let twoDayResult = await client.query({
      query: GLOBAL_DATA(twoDayBlock?.number),
      fetchPolicy: 'cache-first',
    })
    twoDayData = twoDayResult.data.sumswapFactories[0]

    let oneWeekResult = await client.query({
      query: GLOBAL_DATA(oneWeekBlock?.number),
      fetchPolicy: 'cache-first',
    })
    const oneWeekData = oneWeekResult.data.sumswapFactories[0]

    let twoWeekResult = await client.query({
      query: GLOBAL_DATA(twoWeekBlock?.number),
      fetchPolicy: 'cache-first',
    })
    const twoWeekData = twoWeekResult.data.sumswapFactories[0]
...
const getChartData = async (oldestDateToFetch, offsetData) => {
  ...
  try {
    while (!allFound) {
      ...
      skip += 1000
      data = data.concat(result.data.sumswapDayDatas)
      if (result.data.sumswapDayDatas.length < 1000) {
        allFound = true
      }
    }
...
/**
 * Loop through every pair on sumswap, used for search
 */
async function getAllPairsOnSumswap() {
  try {
    let allFound = false
    let pairs = []
    let skipCount = 0
...
/**
 * Loop through every token on sumswap, used for search
 */
async function getAllTokensOnSumswap() {
  try {
    let allFound = false
    let skipCount = 0
    let tokens = []
...
export function useGlobalData() {
  const [state, { update, updateAllPairsInSumswap, updateAllTokensInSumswap }] = useGlobalDataContext()
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      let allPairs = await getAllPairsOnSumswap()
      updateAllPairsInSumswap(allPairs)

      let allTokens = await getAllTokensOnSumswap()
      updateAllTokensInSumswap(allTokens)
    }
    if (!data && ethPrice && oldEthPrice) {
      fetchData()
    }
  }, [ethPrice, oldEthPrice, update, data, updateAllPairsInSumswap, updateAllTokensInSumswap])
...
export function useAllPairsInSumswap() {
...
export function useAllTokensInSumswap() {
  const [state] = useGlobalDataContext()
