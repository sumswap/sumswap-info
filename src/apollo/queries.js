...
export const SUBGRAPH_HEALTH = gql`
  query health {
    indexingStatusForCurrentVersion(subgraphName: "sumswap/sumswap") {
      synced
      health
      chains {
        chainHeadBlock {
...
export const GLOBAL_CHART = gql`
  query sumswapDayDatas($startTime: Int!, $skip: Int!) {
    sumswapDayDatas(first: 1000, skip: $skip, where: { date_gt: $startTime }, orderBy: date, orderDirection: asc) {
      id
      date
      totalVolumeUSD
      dailyVolumeUSD
...
export const GLOBAL_DATA = (block) => {
  const queryString = ` query sumswapFactories {
      sumswapFactories(
